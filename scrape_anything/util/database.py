...
class FileSystemDataBase:
    ...
    def store_prompt(cls, prompt: str, context: str, call_in_seassion: int):
        to_text_file(
            prompt,
            os.path.join(f"{context}", f"step_{str(call_in_seassion)}_prompt.txt"),
        )
    ...
    def store_response(cls, response: str, context: str, call_in_seassion: int):
        to_text_file(
            response,
            os.path.join(f"{context}", f"step_{str(call_in_seassion)}_response.txt"),
        )
    ...
    def store_client_raw_request(cls, obj, context: str, call_in_seassion: int):
        pickle(
            obj, os.path.join(f"{context}", f"step_{call_in_seassion}_raw_request.pkl")
        )
    ...
    def store_server_response(cls, obj, context: str, call_in_seassion: int):
        pickle(
            obj, os.path.join(f"{context}", f"step_{call_in_seassion}_raw_response.pkl")
        )
    ...
    def store_html_elements(cls, raw_on_screen, context: str, call_in_seassion: int):
        dataframe_to_csv(
            raw_on_screen,
            os.path.join(f"{context}", f"step_{call_in_seassion}_raw.csv"),
        )
    ...
    def store_filltered_html_elements(
        ...
        dataframe_to_csv(
            raw_on_screen,
            os.path.join(f"{context}", f"step_{call_in_seassion}_minimized.csv"),
        )
    ...
    def store_exection_status(cls, status_message, context: str, call_in_seassion: int):
        to_text_file(
            status_message,
            os.path.join(
                f"{context}", f"step_{str(call_in_seassion)}_execution_status.txt"
            ),
        )
