...
class LLMInterface(BaseModel):
    ...
    def safe_extract_response(self, response, *args):
        ...
        for arg in args:
            if (
                isinstance(arg, int)
                and isinstance(temp_response, list)
                and arg < len(temp_response)
            ):
                temp_response = temp_response[arg]
            elif (
                isinstance(arg, str)
                and isinstance(temp_response, dict)
                and arg in temp_response
            ):
                temp_response = temp_response[arg]
            else:
                raise LlmProviderError(
                    f"call failed to LLM provider, key {arg} missing from {response}."
                )
