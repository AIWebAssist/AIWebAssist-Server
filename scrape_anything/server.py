...
class Server:
        ...
        try:
            ...
            if data is not None or "session_id" not in data or "user_task" not in data:
                ...
                if session_id not in self.agents_queues:
                    self.init_agent(user_task,session_id,max_message=max_message)
                return self.process_request(params,session_id)
            else:
    ...
    def init_agent(self,user_task,session_id,max_message=-1):
        ...
        self.agents_queues[session_id] = (feed_from_chrome,feed_from_agent,status_feed_queue)
    ...
    def start(self):
        self.app.run(host="0.0.0.0", port=3000, debug=True, use_reloader=False,ssl_context="adhoc")